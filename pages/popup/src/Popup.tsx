...
const Popup = () => {
  ...
  const [currentTabExpanded, setCurrentTabExpanded] = useState(false);
    ...
    chrome.tabs.query({ active: true, lastFocusedWindow: true }, async function (tabs) {
      if (tabs.length > 0) {
        ...
        if (activeTab.url && activeTab.url.startsWith('http')) {
          ...
          const [domainName, tempPort] = await extractDomainAndPort(activeTab.url);
          setDomain(domainName + `${tempPort ? ':' + tempPort : ''}`);
        }
      }
    });
  }, []);
  ...
  const handleAndReload = (tabUrl = activeTabUrl, storageKey = activeStorageKey) => {
    void runWithRefresh(() => handlePull(tabUrl, storageKey, true));
  };
      ...
      <main className="w-full min-w-0 overflow-hidden p-4">
        <Spinner show={false}>
          <VaultList
            ...
            disabled={isBusy}
            onAccountPull={(tabUrl, storageKey) => handleAndReload(tabUrl, storageKey)}
          />

          {domain ? (
            <div className="mt-3 w-full min-w-0 overflow-hidden rounded-md border border-border">
              <button
                type="button"
                className="flex w-full items-center justify-between gap-2 px-3 py-2 text-left hover:bg-accent/40"
                onClick={() => setCurrentTabExpanded(value => !value)}>
                <div className="flex min-w-0 items-center gap-2">
                  <Image src={favIconUrl || ''} value={domain} />
                  <div className="min-w-0">
                    <p className="text-xs font-medium text-muted-foreground">{t('currentTab')}</p>
                    <p className="truncate text-sm font-medium text-foreground">{domain}</p>
                    {showAccountMeta ? (
                      <p className="truncate text-xs text-muted-foreground">{activeAccountMetaLine}</p>
                    ) : null}
                  </div>
                </div>
                <div className="flex shrink-0 items-center gap-1">
                  <Button
                    ...
                    onClick={event => {
                      ...
                      void runWithRefresh(() => requestPush(activeTabUrl, favIconUrl));
                    }}>
                  ...
                  {currentTabExpanded ? <ChevronUp size={16} /> : <ChevronDown size={16} />}
                </div>
              </button>
              ...
              {currentTabExpanded ? (
                <div className="border-t border-border px-3 pb-3 pt-2">
                  ...
                  <Button
                    className="mb-2 justify-start w-full"
                    disabled={isBusy}
                    onClick={() =>
