...
| `WEB_ACCESS_PASSWORD` | Secret | Web / 扩展登录密码（Deploy 时由 deploy-ci.mjs 推送到 Worker） |
...
| Deploy command | `node deploy/cloudflare/deploy-ci.mjs` |

> 勿使用 `… && npx wrangler deploy --config deploy/cloudflare/wrangler.toml`：Dashboard 部署命令字段可能截断路径（如 `.toml` 变成 `.tom`），导致 Wrangler 读不到配置并报 `Missing entry-point`。`deploy-ci.mjs` 在 `deploy/cloudflare` 目录内执行 `wrangler deploy`，路径更短、更稳。
...
| 位置 | 何时生效 | 说明 |
|------|----------|------|
| **Build → Secrets** `WEB_ACCESS_PASSWORD` | 每次 Deploy | `deploy-ci.mjs` 自动执行 `wrangler secret put` |
...
- 将 Deploy command 改为：`node deploy/cloudflare/deploy-ci.mjs`（或 `pnpm deploy:cloudflare-ci`），保存后 **Retry deployment**
...
- Deploy 命令使用 `deploy-ci.mjs`（内含 KV 绑定与 Secret 推送）
