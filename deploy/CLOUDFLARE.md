...
**强烈建议**在 Worker 项目 **Settings → Variables** 设置普通变量 **`SYNC_KV_NAMESPACE_ID`** = 你的 Namespace ID。设置后脚本**跳过查找与创建**，直接绑定该 ID。这样每次 Git 构建都会绑定**同一个** KV，Worker 内保存的 datasource 配置（`__syc_datasource_config__`）不会因 redeploy 而“丢失”。
...
| `SYNC_KV_NAMESPACE_ID` | Variable | **强烈建议** | 固定 SYNC_KV 绑定；设置后跳过 KV 查找/创建 |
| `DEPLOY_ALLOW_KV_CREATE` | Variable | 否 | 设为 `1` 时，未找到命名空间时允许自动创建（Git CI 默认关闭） |
...
| `SYNC_KV_NAMESPACE_ID` | 否 | 固定绑定已有 KV；设置后跳过查找/创建；Git 部署**强烈建议**在 Dashboard 设置 |
