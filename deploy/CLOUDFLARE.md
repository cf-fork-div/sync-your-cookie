...
将 **Web Viewer**（`pages/web`）部署到 Cloudflare Pages，并（可选）自动创建用于扩展同步 Cookie 的 **KV 命名空间**。

> **English summary:** Run `pnpm deploy:cloudflare` for one-command deploy (KV + Pages + credentials output). Web password (`WEB_ACCESS_PASSWORD`) and path (`WEB_BASE_PATH`) are runtime Dashboard settings — no rebuild needed. Alternative: connect this repo to Cloudflare Pages with build command `pnpm install && pnpm build:cloudflare-pages` and output `dist/web` (see [Git deploy](#git-仓库连接部署可选) for limitations).
...
在 [Cloudflare Dashboard](https://dash.cloudflare.com/) → **Workers & Pages** → **Create application** → **Pages** → **Connect to Git**，选择本仓库后配置：

| 设置项 | 值 |
|--------|-----|
| Framework preset | None |
| Build command | `pnpm install && pnpm build:cloudflare-pages` |
| Build output directory | `dist/web` |
| Root directory | `/`（仓库根目录） |
| Node.js version | `20` 或更高 |

`build:cloudflare-pages` 会构建 Web Viewer（`base=/`）并将 `deploy/cloudflare/functions/` 复制到 `dist/web/functions/`，供 Pages Functions 使用。
...
| Push 触发自动构建 | ❌ | ✅ |
| 扩展 Push/Pull（直连 KV API） | ✅ | ✅（与 Pages 部署方式无关） |
...
3. 在 Pages 项目 **Settings → Environment variables → Production** 设置：
   - Secret：`WEB_ACCESS_PASSWORD`
   - Variable：`WEB_BASE_PATH`（默认 `my-cookie-vault`）
4. 将 Account ID / Namespace ID / API Token 填入扩展 Options

> Web Viewer 通过 `/cf-api` 代理访问 Cloudflare REST API，**不依赖** Pages 项目绑定 KV；扩展本身直连 `api.cloudflare.com`，因此 Git 部署不会自动创建 KV 不影响 Web Viewer，但扩展仍需手动配置 KV 凭据。

## 运行时配置（无需重新构建）

Web Viewer 的**登录密码**和**访问路径**由 Pages Functions 在每次请求时从 Cloudflare 环境读取。修改后**立即生效**，无需重新 `pnpm deploy:cloudflare` 或重新触发 Git 构建。

| 变量 | 类型 | 说明 |
|------|------|------|
| `WEB_ACCESS_PASSWORD` | **Encrypted Secret** | Web Viewer 登录密码，**不会**写入前端 JS |
| `WEB_BASE_PATH` | 普通环境变量 | URL 路径段，默认 `my-cookie-vault` → `https://xxx.pages.dev/my-cookie-vault/` |
...
pnpm exec wrangler pages secret put WEB_ACCESS_PASSWORD --project-name sync-your-cookie-web
...
部署脚本会通过 `wrangler pages secret put WEB_ACCESS_PASSWORD` 推送。之后仍可在 Dashboard 随时修改，**无需重新部署**。

> **注意**：`WEB_ACCESS_PASSWORD` **不是**部署必填项。可以先部署，稍后在 Dashboard 补设密码。

### 修改密码后的行为

- 密码校验在 Pages Function `POST /api/login` 服务端完成
...
- 静态资源以 `base=/` 构建，由 `_middleware.ts` 按运行时 `WEB_BASE_PATH` 做路径重写
...
| 变量 | 必填 | 说明 |
|------|------|------|
| `CLOUDFLARE_API_TOKEN` | 推荐 | API Token，需 **Workers KV Storage:Edit** + **Cloudflare Pages:Edit** |
...
| `DEPLOY_RUNTIME_SECRETS` | 否 | 设为 `1` 且已设置 `WEB_ACCESS_PASSWORD` 时，一键部署时自动 `wrangler pages secret put` |
...
```
浏览器
  →  /{WEB_BASE_PATH}/          静态 Web Viewer（middleware 路径重写）
  →  /api/login|session|logout  Pages Functions（httpOnly Cookie 认证）
  →  /cf-api/*                  Pages Function → api.cloudflare.com（需登录，middleware 校验 Cookie）

Chrome 扩展  →  api.cloudflare.com  (直连 KV REST API，不经 Pages)
```

### 认证流程

1. 用户 POST `/api/login` `{ password }`
2. Function 比对 `context.env.WEB_ACCESS_PASSWORD`
3. 成功则设置 `syc_session` httpOnly Cookie（Secure + SameSite=Lax）
4. `/cf-api/*` 与受保护 API 由 `_middleware.ts` 校验 Cookie
...
| `VITE_WEB_BASE_PATH` 构建时固定 base | 运行时 `WEB_BASE_PATH` + middleware 重写 |
...
```bash
# 完整一键部署（KV + Pages + 凭据输出）
pnpm deploy:cloudflare

# 仅构建 Web + 复制 Functions（Git Pages 构建用）
pnpm build:cloudflare-pages

# 仅构建与准备，不创建 KV、不部署 Pages
...
```
deploy/cloudflare/
  wrangler.toml              # Pages 项目 + [vars] WEB_BASE_PATH 初始值
  deploy.mjs                 # 一键部署脚本
  build-pages.mjs            # Git Pages 构建脚本
  functions/
    _middleware.ts           # 路径重写 + Cookie 鉴权
    api/login.ts             # POST 登录
    api/session.ts           # GET 会话状态
    api/logout.ts            # POST 退出
    cf-api/[[path]].ts       # /cf-api 反向代理
...
**Q: Git 连接部署和一键部署选哪个？**  
需要自动创建 KV 并输出扩展凭据 → 用 `pnpm deploy:cloudflare`。需要 push 自动发布 Web Viewer → 用 Git 连接 + `pnpm build:cloudflare-pages`，并手动配置 KV 与 Dashboard 变量。

**Q: 部署后 Web Viewer 无法读写 KV？**  
确认已登录（Cookie 有效），且 `/cf-api` Function 已部署。
