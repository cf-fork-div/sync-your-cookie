...
在开始前，把下面三项抄到记事本（部署后要填 Connect 表单）：

| 信息 | 在哪里找 |
|------|----------|
| **Account ID** | Dashboard 右侧边栏 **Account ID**，或 **Workers & Pages** → 任意 Worker → **Settings** |
| **Namespace ID** | **Workers & Pages** → **KV** → 命名空间 `sync-your-cookie` → 概览页复制（没有则见 [创建 KV](#创建-workers-kv-命名空间)） |
| **API Token** | 自己保存的 `cfut_...`（没有则见 [创建 Token](#创建-api-tokenconnect-表单用)） |

另准备一个 **Web 访问密码**（自定，用于 Web 管理端与扩展登录），稍后加入 Build Secret。
...
| 变量 | 类型 | 必填 | 说明 |
|------|------|------|------|
| `SYNC_KV_NAMESPACE_ID` | Variable | 是 | 第 0 步的 Namespace ID |
...
| `DEPLOY_SEED_DATASOURCE=1` | 首次 Deploy 自动写入 Connect 配置（免手动填 KV 凭据）；覆盖已有配置用 `force` |

`CLOUDFLARE_API_TOKEN` 由 Cloudflare Builds **自动注入**，**不要**手动添加。
...
- [ ] 对本仓库 fork 有 push 权限
- [ ] 已创建 KV 命名空间并记录 **Namespace ID**
- [ ] 已创建 API Token 并记录 **Token 字符串**
...
> **与 Build 自动注入的 Token 区分**：Git 连接部署时，Cloudflare Builds 会自动注入 `CLOUDFLARE_API_TOKEN` 供 `wrangler deploy` 使用，**无需**手动添加到 Build 变量。本节 Token 由你保管，部署后填入 Connect 表单；若 Build 设置了 `DEPLOY_SEED_DATASOURCE=1`，会用自动注入的 Token 预写 Connect，但仍建议保存一份备用。
