...
之后 `pnpm deploy:cloudflare` 会：复用同名 KV `SYNC_YOUR_COOKIE`、推送登录密码 Secret、将 ConnectForm 凭据写入 SYNC_KV。**redeploy 不会清除** Dashboard Secret 或 KV 内已存配置，只要 namespace 绑定不变。
...
**Deploy 前必须运行 `prepare-wrangler.mjs`**：它会按名称 **`SYNC_YOUR_COOKIE`** 查找并复用已有 KV 命名空间（不会重复创建），将真实 ID 写入 `wrangler.toml` 的 `SYNC_KV` 绑定。Cloudflare Builds 会自动注入 `CLOUDFLARE_API_TOKEN`，脚本可自动创建/复用命名空间。

**强烈建议**在 Worker 项目 **Settings → Variables** 设置普通变量 **`SYNC_KV_NAMESPACE_ID`** = 你的 Namespace ID。这样每次 Git 构建都会绑定**同一个** KV，Worker 内保存的 datasource 配置（`__syc_datasource_config__`）不会因 redeploy 而“丢失”。

若已在 Dashboard 手动创建 KV，填入 **`SYNC_KV_NAMESPACE_ID`** 可跳过自动创建并固定绑定。

### Git 部署：Dashboard 一次性变量（推荐）

在 **Workers & Pages → sync-your-cookie → Settings → Variables and Secrets → Production** 设置后，**后续 push 部署无需再手动配置**：

| 变量 | 类型 | 必填 | 说明 |
|------|------|------|------|
| `WEB_ACCESS_PASSWORD` | **Secret** | 是 | Web 登录密码；redeploy **不会**清除 |
| `SYNC_KV_NAMESPACE_ID` | Variable | **强烈建议** | 固定 SYNC_KV 绑定，避免 redeploy 绑到新 namespace |
| `DEPLOY_SEED_DATASOURCE` | Variable | 否 | 设为 `1` 时，部署脚本将 KV 凭据写入 SYNC_KV（ConnectForm 配置） |
| `DEPLOY_RUNTIME_SECRETS` | Variable | 否 | Git 构建**无效**（无 `.env`）；请用 Dashboard Secret 设密码 |
| `CLOUDFLARE_ACCOUNT_ID` | Variable | 否 | 多账户时指定；单账户可省略 |
| `COOKIE_KV_NAMESPACE_ID` | Variable | 否 | Cookie 所在 KV Namespace（默认与 `SYNC_KV_NAMESPACE_ID` 相同） |
| `DATASOURCE_STORAGE_KEY` | Variable | 否 | KV 中的 Cookie 键名，默认 `sync-your-cookie` |
...
| 自动推送 WEB_ACCESS_PASSWORD | ⚠️ 可选（`DEPLOY_RUNTIME_SECRETS=1`） | ❌ 用 Dashboard Secret（一次设置，redeploy 保留） |
...
**Git 部署后仍需手动完成（若未用 Dashboard 变量自动化）：**

1. 确认 Deploy 命令包含 `prepare-wrangler.mjs`（见上表）
2. 在 Worker 项目 **Settings → Variables and Secrets → Production** 设置：
   - Secret：`WEB_ACCESS_PASSWORD`（必填，**一次设置即可**）
   - Variable：**`SYNC_KV_NAMESPACE_ID`**（强烈建议，固定 KV 绑定）
   - Variable（可选）：`DEPLOY_SEED_DATASOURCE=1` — 自动写入 ConnectForm 凭据到 SYNC_KV
   - Variable（可选）：`WEB_BASE_PATH` — 自定义 URL 路径段
...
| `SYNC_KV_NAMESPACE_ID` | 否 | 固定绑定已有 KV；Git 部署**强烈建议**在 Dashboard 设置 |
...
**Q: 再次一键部署会重复创建 KV 吗？**  
不会。脚本按名称 `SYNC_YOUR_COOKIE` 查找并复用；本地还有 `.deploy-state.json` 缓存。Git CI 请设置 Dashboard 变量 **`SYNC_KV_NAMESPACE_ID`** 固定绑定。
