...
同一域名可保存多个账号。在弹窗 **同步账户** 下拉框中选择目标账号，点击 **切换并拉取** 即可一键切换登录态；也可使用 **推送** / **拉取** 手动同步当前标签页。

![扩展弹窗：多账号切换、推送拉取、查看编辑 Cookie](deploy/images/extension-popup.png)

| 步骤 | 操作 |
|------|------|
| **1** | 打开扩展弹窗，在 **同步账户** 下拉框中选择账号（标签 / 文件夹 / 类型） |
| **2** | 点击 **切换并拉取** — 拉取该账号 Cookie 并刷新当前站点 |

### Cookie 增删改查与复制

弹窗中点击 **查看编辑 Cookie** 可管理当前站点 Cookie：

- **添加** — 新增单条 Cookie
- **编辑 / 删除** — 修改或移除已有条目
- **复制** — 复制单条或全部 Cookie
- **JSON 视图** — 以 JSON 格式查看与编辑

（见上图弹窗中的 **查看编辑 Cookie** 入口。）

### 管理器总览

点击 **打开管理器** 进入侧边栏管理界面，查看已推送站点与全局统计。

![管理器：站点列表、文件夹筛选、自动推送](deploy/images/manager-overview.png)

| 步骤 | 操作 |
|------|------|
| **1** | 弹窗底部点击 **打开管理器** |
| **2** | 查看 **Cookie 与 LocalStorage 总计** 及站点列表（host / 文件夹 / 类型 / 自动推送） |
| **3** | 使用文件夹、类型筛选；为单条记录开关 **Auto Push** |

---

## Push / Pull

- **Push** — 上传当前标签页 Cookie（及可选 LocalStorage）；远程已有数据且不一致时弹出冲突对话框（覆盖或另存为新账号）。首次 Push 需填写账号备注；v1.5.1+ 可设置 **文件夹** 与 **类型**（login / session / other）。
- **Pull** — 下载远程 Cookie 并镜像写入浏览器（会先清除该 host 本地 Cookie）。可在弹窗中为每条记录开启 **Auto Pull**。

多账号切换与 Cookie 编辑见上文 [效果预览](#效果预览)；详细行为见 [使用场景与推荐配置](#使用场景与推荐配置)。
...
同站多账号时，使用弹窗 **同步账户** 下拉框 + **切换并拉取**（见 [效果预览](#效果预览)）。各账号的 Auto Push / Auto Pull 开关**独立**，在弹窗或管理器中按条目配置。
